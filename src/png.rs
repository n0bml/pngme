...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let index = self
            ...
            .ok_or_else(|| format!("ChunkTupe '{}' not found!", chunk_type))?;
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        if Png::STANDARD_HEADER != &value[0..8] {
            return Err(format!("Invalid header bytes!").into());
        }
        ...
        while offset < value.len() {
            ...
            offset += chunk.length() + Chunk::METADATA_SIZE;
            chunks.push(chunk);
        }
...
mod tests {
    ...
    fn test_as_bytes() {
        ...
        println!("after Png::try_from");
        let actual = png.as_bytes();
        println!("after png.as_bytes");
        let expected: Vec<u8> = PNG_FILE.iter().copied().collect();
        println!("after PNG_FILE.iter");
